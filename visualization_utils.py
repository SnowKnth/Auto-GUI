...
def _plot_action(
  ...
  if ex_action_type == action_type.ActionType.DUAL_POINT.name:
    return _plot_dual_point(
        touch_x, touch_y, lift_x, lift_y, screen_height, screen_width, ax
    )
  elif ex_action_type in (
      action_type.ActionType.PRESS_BACK.name,
      action_type.ActionType.PRESS_HOME.name,
      action_type.ActionType.PRESS_ENTER.name,
  ):
    text = ex_action_type
    _add_text(text, screen_width, screen_height, ax)
  elif ex_action_type == action_type.ActionType.TYPE.name:
    text = f'Input text "{action_text}"'
    _add_text(text, screen_width, screen_height, ax)
  elif ex_action_type == action_type.ActionType.STATUS_TASK_COMPLETE.name:
    text = 'Set episode status as COMPLETE'
    _add_text(text, screen_width, screen_height, ax)
  elif ex_action_type == action_type.ActionType.STATUS_TASK_IMPOSSIBLE.name:
...
def plot_example(
  ...
  if show_action:
    ...
    ax = _plot_action(
        action_type.ActionType(ex_action_type).name,
        image_height,
        image_width,
        touch_x,
        touch_y,
...
def plot_example_from_stored(
  ...
  Returns:
    The matplotlib axis that the example was plotted on.
  """
  image_height = episode['height_width_channels'][0]
  image_width = episode['height_width_channels'][1]
  image = episode['image'] 
  ...
  if show_annotations:
    positions = (episode["ui_positions"]* [
      image_height,
      image_width,
      image_height,
      image_width,]).astype(int)
    for y, x, h, w in positions:
      rect = patches.Rectangle(
          (x, y), w, h, linewidth=1, edgecolor='r', facecolor='none'
      )
  ...
  if show_action:
    touch_y, touch_x = episode["result_touch_yx"]
    lift_y, lift_x = episode["result_lift_yx"]
    ex_action_type, type_text = episode["result_action"]
    ax = _plot_action(
        ex_action_type,
        image_height,
        image_width,
